...
export default function Portfolio({ firstStack }: PortfolioProps) {
  ...
  return (
    <section
      id="portfolio"
      // PERBAIKAN: Background disesuaikan untuk dark mode
      className="w-full py-20 md:py-24 bg-slate-100 dark:bg-slate-900 transition-colors duration-500"
    >
      <div className="max-w-6xl mx-auto px-4">
        <header className="text-center mb-10 md:mb-14">
          <p className="text-sky-600 dark:text-sky-400 text-sm tracking-wide font-semibold">Portfolio</p>
          {/* PERBAIKAN: Warna text judul */}
              ...
              <div className="p-4 md:p-5">
                ...
                <div className="mt-5 flex flex-wrap items-center gap-2">
                  {p.tech?.map((t, idx) => (
                    <span
                      key={idx}
                      className="inline-flex items-center gap-2 rounded-full border px-2.5 py-1 text-xs transition-colors bg-slate-100 dark:bg-slate-900 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300"
                    >
                      <Image
                        src={t.img}
                        alt={t.name}
                ...
                <div className="mt-6 flex items-center gap-3">
                  <a
                    href={p.live}
                    className="inline-flex items-center justify-center rounded-xl px-4 py-2.5 text-sm font-medium text-white bg-sky-600 hover:bg-sky-700 transition shadow-sm"
                  >
                    Live Demo
                  </a>
                  <a
                    href={p.repo}
                    className="inline-flex items-center justify-center rounded-xl px-4 py-2.5 text-sm font-medium text-slate-700 dark:text-slate-300 border border-slate-300 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-700 transition"
                  >
                    Source Code
                  </a>
                </div>
